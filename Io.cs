...
class Io
{
            ...
            switch (command_syntax[0])
            {
                ...
                case "go":
                    switch (command_syntax[1])
                    {
                        case "ponder":
                            if (!game.IsPlaying)
                                Move = treesearch.MultithreadMcts(game.Board, (byte)game.Turn, Int32.MaxValue, game.NNUE, game.HalfKp, game.HalfKav2, game.TreadCount, true, false , 0);
                            if (Move != null)
                        ...
                        case "infinite":
                            ...
                            break;
                        case "movetime":
                            Move = treesearch.MultithreadMcts(game.Board, (byte)game.Turn, Int32.MaxValue, game.NNUE, game.HalfKp, game.HalfKav2, game.TreadCount, true, true, Convert.ToInt64(command_syntax[2]));
                            if (Move != null)
                                ReturnMoove(Move[0], Move[1]);
                            break;
                        case "perft":
                        ...
                        case "depth":
                            ...
                            break;
                        case "nodes":
                            Move = treesearch.MultithreadMcts(game.Board, (byte)game.Turn, Convert.ToInt32(command_syntax[2]), game.NNUE, game.HalfKp, game.HalfKav2, game.TreadCount, false , false , 0);
                            if (Move != null)
                                ReturnMoove(Move[0], Move[1]);
                            break;
                        default:
    ...
    {
        game.training = new Training(game.Board, game.NetName, game.BufferSize, game.GameLength, game.NodeCount, game.Coefficient, game.TreadCount, game.Momentum, game.TrainingSampleSize, game.NetDecay, game.Elo, game.Lambda);
    }
}
