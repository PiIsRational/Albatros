...
{
    PSQT psqt = new PSQT();
    public float EndgameImportance(int Piececount , int KingX , int KingY , int X , int Y , int color , bool pawn)
    {
        float Importance = 0;
        if (Piececount < 11)
            Importance = 1;

        float LargestDistance = (KingX - X);
        if (LargestDistance < 0)
            LargestDistance = -LargestDistance;
        if (LargestDistance * LargestDistance < (KingY - Y) * (KingY - Y))
            LargestDistance = (KingY - Y);
        if (LargestDistance < 0)
            LargestDistance = -LargestDistance;

        return Importance * (-LargestDistance + 8) / 14;
    }
    public float EndgameKingPlace(int Piececount, int BlackX, int BlackY, int WhiteX, int WhiteY)
    {
        float Importance = 0;
        ...
        double ScoreWhite = DistanceFromEdge(WhiteX, WhiteY);
        double ScoreBlack =- DistanceFromEdge(BlackX, BlackY);

        return (float)(ScoreWhite + ScoreBlack) * Importance / 8;
    }
