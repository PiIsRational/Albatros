...
class Treesearch
{
    ...
    Semaphore StopSemaphore = new Semaphore(1, 1);
    ...
    public Treesearch(int seed, bool LoadNet ,int ThreadCount)
    {
        ...
        if (LoadNet)
        {
            ...
            catch
            {
                ...
                ValueNet.SaveNet("ValueNet.nnue", false);
            }
        }
        if(ThreadCount > 1)
        {
            ThreadTreesearches = new BranchThread[ThreadCount - 1];
    ...
    public void Test(int iterations, byte[,] Position, byte[,] Position2)
    {
        for (int i = 0; i < iterations; i++)
        {
            //Backpropagate the network
            //ValueNet.BackPropagation2(new byte[][][,] { new byte[][,] { Position, new byte[,] { { 1 } }, new byte[,] { { 0 } } }, new byte[][,] { Position2, new byte[,] { { 0 } }, new byte[,] { { 1 } } } }, 0.01f);
            //  ValueNet.setNet(new double[][][][,] { ValueNet.WeigthChanges }, new double[][][][] { ValueNet.BiasChange }, new double[][,] { ValueNet.MatrixChange });
            //Test the network
         //   double Value = ValueNet.CostOfNet(new byte[][][,] { new byte[][,] { Position, new byte[,] { { 1 } }, new byte[,] { { 0 } } }, new byte[][,] { Position2, new byte[,] { { 0 } }, new byte[,] { { 1 } } } });
       //     Console.WriteLine("The Current Error is : {0} ", Value);
        }
    }
    ...
    public MCTSimOutput MonteCarloTreeSim(byte[,] InputBoard, byte color, int NodeCount, bool NewTree, bool Random, bool NNUE)
    {
        MCTSimOutput Output = new MCTSimOutput();

        if (NewTree)
            CurrentTree = GetTree(NodeCount, false, NNUE, new Node(InputBoard, color , null , 0)).Tree;
        else
            CurrentTree = GetTree(NodeCount, false, NNUE, CurrentTree).Tree;
    ...
    public int[][] MultithreadMcts(byte[,] InputBoard, byte color, int NodeCount, bool NNUE, int ThreadCount)
    {
        ...
        int Rest = 0;
        int BranchesByThread;
        Thread[] ThreadPool = new Thread[ThreadCount - 1];
        Stopwatch sw = new Stopwatch();
        bool NotFinished = true;
        sw.Start();
        Node Tree;
        BranchThread MainBranch = new BranchThread();
        ...
        else
        {
            ...
            TreeGenOut = GetTree(1, false, NNUE, new Node(InputBoard, color, null, -2));
        ...
        int[] ChildNodecounts = new int[TreeAmount];
        int[] ChildNodeNodeCount = new int[TreeAmount];
        BranchesByThread = TreeAmount / ThreadCount;
        Rest = TreeAmount % ThreadCount;
        for (int i = 0; i < ThreadCount; i++)
        {
            if (Rest != 0)
            {
                BranchesPerThread[i] = BranchesByThread + 1;
                Rest--;
            }
            else
                BranchesPerThread[i] = BranchesByThread;
        }
        for (int i = 0; i < TreeAmount; i++)
            ChildNodecounts[i] = (int)Tree.ChildNodes[i].Denominator;

        ChildNodecounts = SortArray(ChildNodecounts);
        int Max = ChildNodecounts[TreeAmount - 1];

        for (int i = 0; i < TreeAmount - 1; i++)
        {
            if (NodeCount > 0)
                ChildNodeNodeCount[i] = Max - ChildNodecounts[i];
            NodeCount -= Max - ChildNodecounts[i];
        }
        if (!(Tree.Denominator >= NodeAmount))
        {
            for (int l = 0; l < 2; l++)
            {
                if (ChildNodeNodeCount[0] == 0)
                    l = 1;
                //init
                for (int i = 0; i < TreeAmount; i++)
                {
                    int CurrentCount = ChildNodeNodeCount[i];
                    if (l == 1)
                        CurrentCount = NodeCount / TreeAmount;
                    Branches[i] = new BranchThreadInput(Tree.ChildNodes[i], CurrentCount, NNUE);
                }
                //Leveling
                int Counter = 0;
                for (int i = 0; i < ThreadCount; i++)
                {
                    if (i == ThreadCount - 1)
                    {
                        ...
                        MainBranch.Nodecount = Branches[i].NodeAmount;
                    ...
                    else
                    {
                        ...
                        ThreadTreesearches[i].Nodecount = Branches[i].NodeAmount;
                ...
                //Output first Info
                Console.WriteLine("info depth {0} seldepth {1} time {4} nodes {5} score cp {2} nps {3}", (int)(AverageDepth / Tree.Denominator), Seldepth, Math.Round(CentipawnLoss(Tree.Numerator / Tree.Denominator)), (int)(((float)Tree.Denominator / (sw.ElapsedMilliseconds + 1)) * 1000), sw.ElapsedMilliseconds, Tree.Denominator);
            }
        }
    ...
    public TreesearchOutput GetTree(int NodeCount, bool Output, bool NNUE, Node startNode)
    {
        Node Tree = startNode;
        NodeCount -= (int)startNode.Denominator;
        Node CurrentNode;
    ...
    public int[] MinMaxAlphaBeta(byte[,] InputBoard, byte color, int depthPly , bool NNUE )
    {
            ...
            foreach (int[] Move in CleanedMoves) 
            {
                ...
                if (!CompleteCheck(InputBoard, Othercolor))
                {
                    if (MatingValue == 2)
                    {
                        if (CurrentScore == -2)
                        {
                            CurrentScore = -MinMaxAlphaBetaScore(InputBoard, Othercolor, depthPly - 1, BestScore, false, NNUE);
                            if (CurrentScore != -2)
                            {
                                Output = CleanedMoves.ToArray()[counter];
                                BestScore = CurrentScore;
                        ...
                        {
                            CurrentScore = -MinMaxAlphaBetaScore(InputBoard, Othercolor, depthPly - 1, BestScore, true, NNUE);
                            if (BestScore < CurrentScore)
                            {
                                Output = CleanedMoves.ToArray()[counter];
                                BestScore = CurrentScore;
    ...
    public double MinMaxAlphaBetaScore(byte[,] InputBoard, byte color, int depthPly, double LastBest, bool Activation , bool NNUE)
    {
        List<int[]> Moves = ReturnPossibleMoves(InputBoard, color);
    ...
    public byte[,] KingCastleExecuteMooves(byte[,] InputBoard, int[] Move, int X, int Y)
    {
        ...
        else
        {
            ...
            if (ReturnPossibleMoves(InputBoard, EnemyColor) != null)
            {
                ...
                if (X - Move[0] == 2)
                {
                    UnmakeMove = new int[] { X, Y, Copy - 1, Move[0], Move[1], 0, X - 1, Y, InputBoard[1, Y], 1, Y, 0 };
                ...
                {
                    UnmakeMove = new int[] { X, Y, Copy - 1, Move[0], Move[1], 0, X + 1, Y, InputBoard[8, Y], 1, Y, 0 };
    ...
    public List<byte[,]> KingStartExecuteMooves(byte[,] InputBoard, List<int[]> Mooves, int X, int Y)
    {
        byte Copy = (byte)(InputBoard[X, Y] + 1);
        List<byte[,]> Output = new List<byte[,]>();
        byte[,] CurrentBoard = new byte[9, 9];
        foreach (int[] Moove in Mooves)
        {
            Array.Copy(InputBoard, 0, CurrentBoard, 0, InputBoard.Length);
            if (X - Moove[0] != 2 && X - Moove[0] != -2)
            {
                CurrentBoard[X, Y] = 0;
                CurrentBoard[Moove[0], Moove[1]] = Copy;
                Output.Add(CopyArray(CurrentBoard));
            }
            // if Castelling
            else
            {
                ...
                if (KingColor == 0)
                    EnemyColor = 1;
                //Copy the King to two new Squares
                CurrentBoard[(X + Moove[0]) / 2, Y] = Copy;
                CurrentBoard[Moove[0], Moove[1]] = Copy;
                //Check if any of the Kings are in Check
                if (!CompleteCheck(CurrentBoard, EnemyColor))
                {
                    ...
                    if (X - Moove[0] == 2)
                    {
                        CurrentBoard[X - 1, Y] = (byte)(InputBoard[1, Y] + 1);
                        CurrentBoard[1, Y] = 0;
                        Output.Add(CopyArray(CurrentBoard));
                    }
                    //KingSideCastle
                    ...
                    {
                        CurrentBoard[X + 1, Y] = (byte)(InputBoard[8, Y] + 1);
                        CurrentBoard[8, Y] = 0;
                        Output.Add(CopyArray(CurrentBoard));
                    }
                }
    ...
    public byte[,] CopyArray(byte[,] Input)
    {
...
{
    public Node Tree;
    public int Seldepth, averageDepth;
}
class MCTSimOutput
{
...
class BranchThread
{
    ...
    public void StartThread()
    {
        ...
        for (int i = 0; i < Trees.Length; i++)
        {
            ...
            if (Trees[i].Denominator >= 0)
                Outputs[i] = treesearch.GetTree(Nodecount, false, NNUE, Trees[i]);
            else
                Console.WriteLine("Error!");           
        }
