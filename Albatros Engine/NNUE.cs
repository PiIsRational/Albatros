...
class NNUE
{
    ...
    int[] Format = new int[] { 1024, 16, 32, 1 };
    ...
    public NNUE()
    {
        ...
        for (int i = 0; i < Biases.Length; i++)
        {
            ...
            {
                BiasChange[i][j] = new float[Format[j + 1]];
                OldBiases[i][j] = new float[Format[j + 1]];
                Biases[i][j] = new float[Format[j + 1]];
                NeuronVal[j] = new float[Format[j + 1]];
                NeuronInput[j] = new float[Format[j + 1]];
                NeuronErrors[j] = new float[Format[j + 1]];
                for (int k = 0; k < Biases[i][j].Length; k++)
                {
                    Biases[i][j][k] = 0;//Convert.ToSingle(((float)random.NextDouble() - 0.5) * 1 / 2);
                }
            }
        }
        // init Weights
        for (int i = 0; i < Weigths.Length; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                OldWeigths[i][j] = new float[Format[j], Format[j + 1]];
                WeigthChanges[i][j] = new float[Format[j], Format[j + 1]];
                Weigths[i][j] = new float[Format[j], Format[j + 1]];
                for (int k = 0; k < Weigths[i][j].GetLength(0); k++)
                {
                    for (int l = 0; l < Weigths[i][j].GetLength(1); l++)
                    {
                        Weigths[i][j][k, l] = 0;//Convert.ToSingle(((float)random.NextDouble() - 0.5) * 1 / 2);
                    }
                }
            }
        }
        ...
        for (int i = 0; i < StartMatrix.GetLength(0); i++)
        {
            ...
            {
                StartMatrix[i, j] = 0;//Convert.ToSingle(((float)random.NextDouble() - 0.5) * 1 / 2);
            }
        }
        PSQTInit();
    }
