...
class StandartChess
{
    ...
    {
        return (int)Math.Round(Standart.InverseSigmoid(input, 4.2f) * 100);
    }
    ...
    public static bool NonMateWindow(int alpha, int beta)
    {
        return alpha > -mate_value && beta < mate_value;
    }

    public static int WDLToMillipawn(float input)
    {
        return Math.Min(mate_value - 1, Math.Max(1 - mate_value, (int)Math.Round(Standart.InverseSigmoid(input, 4.2f) * 1000)));
    }

    public static float MillipawnToWDL(int input)
    {
        if (Math.Abs(input) >= mate_value)
            return input / mate_value;
        return Standart.Sigmoid((float)input / 1000, 4.2f);
    }
    public static Accumulator AccCopy(Accumulator input)
    {
        Accumulator ouput = new Accumulator(input.accu[0].Length);

        Array.Copy(input.accu[0], ouput.accu[0], input.accu[0].Length);
        Array.Copy(input.accu[1], ouput.accu[1], input.accu[1].Length);
