...
class ZobristHash
{
    ...
    public ulong HashPosition(Position board)
    {
        ...
        if (board.enPassentSquare != 255)
            output ^= en_passent[chess_stuff.GetY(board.enPassentSquare)];
    ...
    public ulong UpdateHashAfterMove(Position board, ReverseMove unmake_move, ulong hash)
    {
        ...
        if(unmake_move.enPassent != byte.MaxValue)
            hash ^= en_passent[chess_stuff.GetY(unmake_move.enPassent)];

        //add the new en passent square
        if (board.enPassentSquare != byte.MaxValue)
            hash ^= en_passent[chess_stuff.GetY(board.enPassentSquare)];
