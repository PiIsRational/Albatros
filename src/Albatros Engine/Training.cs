...
class Training
{
    ...
    string CurrentNetName = "start-0.nn";
    ...
    public Training(byte[,] Position, string EvalFileName, int BufferSize, int GameLength, int NodeCount, float LearningCoefficient, int ThreadAmount, float Momentum, int SampleCreateSize, int Batchsize, float NetDecay, int startElo, float WinToEvalCoefficient)
    {
        //Change the elo
    ...
    public double GetLearningRate(double error)
    {
        return LearningRate * Math.Pow(10, Math.Log(error, Math.E));
    }
    ...
    public void ManageTraining()
    {
        ...
        while (true)
        {
            ...
            {
                //Test
                treesearch.Test(1000, CurrentTrainingSet[0]);
                ...
                {
                    cost = TrainNet.CostOfNet(GetRandomSampleFromBuffer(10000));
        ...
        while (Work)
        {
            ...
            for (int i = 0; i < GameLength; i++)
            {
                ...
                if (ToPlay)
                {
                    ToPlay = false;
                    //Play the Moove
                    Array.Copy(treesearchOfficial.MonteCarloTreeSim(Board, Color, 300, true, start, true).Position, Board, Board.Length);
                }
                else
                {
                    ToPlay = true;
                    //Play the Moove
                    Array.Copy(treesearchTesting.MonteCarloTreeSim(Board, Color, 300, true, start, true).Position, Board, Board.Length);
                }
    ...
    public void GameplayThreadStart()
    {
        ...
        while (Work)
        {
            ...
            for (int i = 0; i < GameLength; i++)
            {
                ...
                simOutput = treesearchV1.MonteCarloTreeSim(Board, Color, NodeCount, NewStart, start, false);
